# Project Agent Instructions

## Canonical generic workflow

Follow the project's adopted copy or pinned reference to `GENERIC_CODEX_WORKFLOW.md`. In particular: use minimum sufficient scope, inspect targeted files, preserve unrelated work, choose the lowest credible validation tier, respect artifact roles, check Git status, and do not push unless requested.

Canonical workflow location: `[path or URL]`

## PROJECT-SPECIFIC RULES

<!-- Add only rules unique to this project. Keep product decisions out of the generic workflow. -->

### Sources of truth

- Design source: `[path]`
- Process/production source: `[path]`
- Approved-state index: `[path]`
- Canonical release artifacts, if any: `[path and policy]`

### Project commands

- Build: `[command]`
- Focused checks: `[command]`
- Export/package: `[command]`

### Validation entry points

- Small/local: `[command or document]`
- Geometry/behavior: `[command or document]`
- Manufacturing/export: `[command or document]`
- Production checkpoint/release: `[command or document]`
